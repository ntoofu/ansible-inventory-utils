...
class AnsibleComparatorTestCase(unittest.TestCase):
    def setUp(self):
        ...
        self.inventory = [
                load_ansible_inventory(config["base_dir"],
                                       config["inventory_file"],
                                       config["vault_pass"])
                for config in self.inventory_config]
    ...
    def test_different_inventories(self):
        different_index_pairs = [(x, y)
                                 for x in range(0, len(self.inventory))
                                 for y in range(0, len(self.inventory))
                                 if x < y]
        results = []
        for index_pair in different_index_pairs:
            inv1 = self.inventory[index_pair[0]]
            inv2 = self.inventory[index_pair[1]]
            results.append(diff_ansible_inventory(inv1, inv2, True))
